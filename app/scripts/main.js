...
(function(global, document, $, d3) {
  ...
  MessiViz.renderMinutesMatrix = function() {
    ...
    var matrixContainer = MessiViz.groups.matrix
        .selectAll(".matrix-container")
        .data(goalsMinutes);
    ...
    MessiViz.groups.matrix
        ...
        .each(function(d,a){
          ...
          container.select(".matrix-back")
            .transition()
            .attr("fill", function(d) {
              return MessiViz.colorMinutes(d.values.length);
            });

          container.select(".matrix-goals")
            .text(function(d) {
              return d.values.length;
            });

          container.select(".matrix-minute")
            .text(function(d) {
              return d.key == 90 || d.key == 45 ? d.key + "'+" : d.key + "'";
            });
        });
